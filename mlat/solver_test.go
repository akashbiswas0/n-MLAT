...
func TestSolveRejectsOutlierAndKeepsInliers(t *testing.T) {
	sensors := []struct {
		id  int64
		lat float64
		lon float64
	...
	target := GeodeticToECEF(50.2000, -5.2000, 7000)
	emissionNs := int64(700_000_000_000)
	...
	for i, sensor := range sensors {
		...
		obs = append(obs, Observation{
			...
			SecondsSinceMidnight: uint64(arrivalNs / 1_000_000_000),
			Nanoseconds:          uint64(arrivalNs % 1_000_000_000),
			RawModeS:             []byte{0x8d, 0xab, 0xc1, 0x23, 0x02},
		})
	}
	...
	if got.NumSensors != 4 {
		t.Fatalf("expected one outlier to be rejected, got %d inliers", got.NumSensors)
	}
	if diff := absFloat(got.Lat - 50.2000); diff > 0.08 {
		t.Fatalf("latitude too far off after outlier rejection: got %.6f diff %.6f", got.Lat, diff)
	}
	if diff := absFloat(got.Lon - (-5.2000)); diff > 0.08 {
		t.Fatalf("longitude too far off after outlier rejection: got %.6f diff %.6f", got.Lon, diff)
	}
}
